...
export type Membre = { id: string; nom: string; role: 'admin' | 'membre'; cooperative_id: string; telephone?: string; user_id?: string };
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const signUpCreateCoop = useCallback(async (p: any) => {
    ...
    const codeCoop = `COOP-${Date.now().toString().slice(-6)}-${Math.random().toString(36).substr(2,3).toUpperCase()}`;
    const { data: c, error: e3 } = await supabase.from('cooperatives')
      .insert({ nom: p.coopNom, ville: p.ville, telephone: p.telephone, admin_id: uid, code_coop: codeCoop })
      .select().single();
    if (e3) throw e3;
  ...
  const signUpJoinCoop = useCallback(async (p: any) => {
    const { data: coopIdData, error: ec } = await supabase.rpc('find_coop_id_by_code', { p_code: p.code_coop.trim() });
    if (ec || !coopIdData) throw new Error('Code coopérative introuvable');
    const { data: signUp, error } = await supabase.auth.signUp({ email: p.email.trim(), password: p.password });
    if (error) throw error;
    let uid = signUp.user?.id;
    if (!signUp.session) {
    ...
    if (!uid) throw new Error('Création de compte impossible');
    const { error: e4 } = await supabase.from('membres')
      .insert({ user_id: uid, cooperative_id: coopIdData, nom: p.nom, telephone: p.telephone, role: 'membre' });
    if (e4) throw e4;
    await refresh();
  }, [refresh]);
